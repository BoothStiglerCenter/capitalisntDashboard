...
setwd('C:/Users/Joshualevy/Documents/Capitalisnt Analysis/07-14')

csv_files <- list.files(path='.', pattern='Capitalisnt Data Simplecast (.*)_\\d{2}\\.\\d{2}\\.\\d{4}\\.csv')


df <- data.frame()

for (file in csv_files){
  temp_df <- read_csv(file=file)
  df <- rbind(df, temp_df)
}
